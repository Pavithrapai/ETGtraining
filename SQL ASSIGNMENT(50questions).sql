...
SELECT e.ENAME AS 'EMPLOYEE',e.HIREDATE,m.ENAME AS 'MANAGER',m.HIREDATE
FROM EMP e,EMP m
...
SELECT COUNT(JOB) AS 'NO OF MANAGERS'
FROM EMP
WHERE JOB = 'MANAGER';
...
SELECT FORMAT(HIREDATE, 'MM-dd-yy') AS 'FORMATTED DATE'
FROM EMP;
...
SELECT JOB,DEPTNO,
AVG(SAL*12) AS 'AVERAGE ANNUAL SAL'
FROM EMP
GROUP BY JOB ,DEPTNO;
...
SELECT COUNT(SAL) AS 'RECEIVE SAL',
COUNT(COMM) AS 'RECEIVE COMMISSION'
FROM EMP
WHERE DEPTNO = 30 ;
...
SELECT AVG(SAL) AS 'AVERAGE SALARY',MIN(SAL) AS 'MINIMUM SALARY',MAX(SAL) AS 'MAXIMUM SALARY'
FROM EMP
...
SELECT DEPTNO
FROM EMP
WHERE EXISTS (SELECT COUNT(JOB) AS 'NO OF CLERKS'
               FROM EMP
               WHERE JOB = 'CLERK'
               GROUP BY DEPTNO)
               GROUP BY DEPTNO;
...
SELECT ENAME,SAL
FROM EMP
WHERE SAL < (SELECT SAL*0.3 AS 'SAL LESS THEN 30%'
              FROM EMP
			 WHERE JOB= 'PRESIDENT');

--26. How many employees work in Chicago?

SELECT COUNT(ENAME) AS 'NO OF EMPLOYEES'
FROM EMP
JOIN
DEPT ON LOC='CHICAGO';
...
SELECT COUNT(DISTINCT EMPNO) AS 'DIFFERENT EMPLOYEES',COUNT(DEPTNO) AS 'NO OF DEPT'
FROM EMP;
...
select  e.deptno,dname,loc,sum(comm) as 'total commission',sum(sal) as 'total salary of each dept'
from emp e,dept d
...
select  top 1 ename,min(sal) as 'min_salary'
from EMP  
group by ename
order by min_salary;

select top 1 ename,max(sal) as 'max_salary'
from EMP 
group by ename
...
select ename as 'Employee',sal as 'Salary',job as 'Job',hiredate as 'Date of Hire',comm as 'Commission',deptno as 'Department No',mgr as 'MGR No'
from EMP;


--47. What is the earliest date on which someone was hired, and what is the latest date on which someone was hired.

select e.ename as 'Earliest Date' , c.ename as 'Latest Date'
from emp e , emp c
